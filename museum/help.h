...
public:
    Material();
    Material(const std::string &name, Vec4 &ambient, Vec4 &diffuse, Vec4 &specular, float &shininess);

public:
    std::string _name;

    Vec4 _ambient, _diffuse, _specular;
    float _shininess;
};

class Obj3D : public Node {
public:
    static Obj3D * create(const std::string &file, const std::string &vsh, const std::string &fsh);
...
protected:
    ...
    bool loadMtlFile(const std::string &file);
